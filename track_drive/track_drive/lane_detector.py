...
class LaneDetector:
    def __init__(
        ...
        self.use_lookahead = use_lookahead  # True: ROI 상단 가중치 ↑ (먼 곳)
    ...
    def _line_x(self, mask, x_min=0, x_max=None):
        ...
        use_lookahead=False: 하단 가중치 ↑ (지금 위치)
        use_lookahead=True : 상단 가중치 ↑ (앞으로 갈 위치)"""
        if x_max is None:
            x_max = mask.shape[1]
...
def _cli():
    ...
    det = LaneDetector(target_lane=args.lane, use_lookahead=args.lookahead)
